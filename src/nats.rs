...
//! `tripbot.<env>.playout.<verb>.<platform>`); the currently-playing clip and
//! playback position flow back through the `TRIPBOT_PLAYOUT_LASTPLAYED`
//! JetStream last-value cache, which a restarted instance reads to resume where
//! it left off. Both names are the wire contract tripbot's playout-client and
//! the console speak.
...
impl Control {
    ...
    /// leaf (`tripbot.<env>.playout.<verb>.<platform>`) — the shape tripbot
    ...
    pub async fn run_commands(self: Arc<Self>, player: SharedPlayer) {
        ...
        let base = subject(&self.env, ""); // "tripbot.<env>.playout."
...
/// Command verb from a full subject: strips the `tripbot.<env>.playout.`
...
#[cfg(test)]
mod tests {
    use super::{LASTPLAYED_STREAM, subject, verb_of};

    /// The wire names are the contract tripbot and the console speak; they
    /// must stay byte-exact.
    #[test]
    fn wire_names_match_the_contract() {
        assert_eq!(subject("prod", "skip"), "tripbot.prod.playout.skip");
        assert_eq!(LASTPLAYED_STREAM, "TRIPBOT_PLAYOUT_LASTPLAYED");
    }

    #[test]
    fn verb_of_strips_base_and_platform_leaf() {
        let base = "tripbot.production.playout.";
        assert_eq!(
            verb_of(
                "tripbot.production.playout.play.random.youtube",
                base,
                "youtube"
            ),
            Some("play.random")
        ...
        assert_eq!(
            verb_of("tripbot.production.playout.skip.youtube", base, "youtube"),
            Some("skip")
        );
        // Another platform's leaf must not dispatch here.
        assert_eq!(
            verb_of("tripbot.production.playout.skip.twitch", base, "youtube"),
            None
        );
        // Bare verb without a platform leaf is not a command.
        assert_eq!(
            verb_of("tripbot.production.playout.skip", base, "youtube"),
            None
        );
