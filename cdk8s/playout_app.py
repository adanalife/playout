...
(/playout/current for tripbot/console reads, health probes, /version) — the
name tripbot's `VLC_SERVER_HOST` points at. That name and that port come from
contract.json, generated by tripbot's `pkg/contract` and synced in via
`task contract:sync`.
...
import cdk8s
from config import PORTS, SERVICES, EnvConfig
from constructs import Construct

IMAGE = "ghcr.io/adanalife/playout"
PART_OF = "tripbot"
CONFIG_HASH_ANNOTATION = "adanalife.dev/config-hash"
HTTP_PORT = PORTS["playout_http"]  # the binary's HTTP_PORT default
...
class PlayoutInstance(Construct):
    def __init__(
        ...
        *,
        ...
        name = SERVICES[f"playout_{platform}"]
